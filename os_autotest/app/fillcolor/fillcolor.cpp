...
int main(int argc, char* argv[])
{
    ...
    // Draw filled contours on black image
    Mat result = Mat::zeros(gray.size(), CV_8UC1);
    drawContours(result, contours, -1, Scalar(255), cv::FILLED);

    // Save the result image
    imwrite("ip_fill.png", result);
...
int main1()
{
    ...
    // Fill hollow number with black color
    Mat result;
    bitwise_and(gray, gray, result, inverted);
...
Mat get_ch3_green(Mat src,Rect color_rec){

    Rect ch_rec = Rect(1280,0,640,360);

    Mat src_ch1 = getcolormask(src(ch_rec),3);
    //prev_mask_ch1 = getbluemask(prev(ch1_rec));
    Mat element = getStructuringElement(MORPH_RECT,Size(3,3));
    Mat src_ch1_mor;
    morphologyEx(src_ch1, src_ch1_mor,MORPH_OPEN, element);

    Mat roi = src_ch1_mor(color_rec);
    return roi;
}

Mat get_ch3_orange(Mat src,Rect color_rec){

    Rect ch_rec = Rect(1280,0,640,360);

    Mat src_ch1 = getcolormask(src(ch_rec),2);
    //prev_mask_ch1 = getbluemask(prev(ch1_rec));
    Mat element = getStructuringElement(MORPH_RECT,Size(3,3));
    Mat src_ch1_mor;
    morphologyEx(src_ch1, src_ch1_mor,MORPH_OPEN, element);
...
int main0(int argc, char* argv[])
{
    ...
    Rect color_rec = Rect(140,140,350,140);
    Mat first_roi = get_ch3_green(screen,color_rec);
    ...
    Mat mat{};
    Mat gray{};
    int changenum=0,level,threshold=200,switchnum=0,average=colornum,count=0,colornum_o;
    ...
    for(;;)
    {
        ...
        roi = get_ch3_green(now,color_rec);
        colornum = countNonZero(roi);
        //roi_o = get_ch3_orange(now,color_rec);
        //colornum_o = countNonZero(roi_o);
        ...
        //fprintf(stderr,"color_o num : %d ,",colornum_o);
    ...
    Rect led_pos = Rect(220,160,160,60);
    Mat ch_img = now(rec);
    cv::imwrite("ledcheck_g3.png", ch_img, {cv::IMWRITE_PNG_COMPRESSION, 9});
    Mat color_img = getcolormask(ch_img,3);
    Mat element = getStructuringElement(MORPH_RECT,Size(3,3));
    ...
    cv::imwrite("ledcheck_g3_mask.png", color_morph, {cv::IMWRITE_PNG_COMPRESSION, 9});

    Mat ch_img2 = ch_img;
    rectangle(ch_img,led_pos,Scalar(0,0,255),5,1,0);
    cv::imwrite("ledcheck_g3_rect.png", ch_img, {cv::IMWRITE_PNG_COMPRESSION, 9});
    rectangle(ch_img2,color_rec,Scalar(0,0,255),5,1,0);
    cv::imwrite("ledcheck_g3_rect2.png", ch_img, {cv::IMWRITE_PNG_COMPRESSION, 9});

    int remote_on = switchnum>=2 ? 1 : 0;

    Mat orange_roi = get_ch3_orange(screen,color_rec);
    int orange_colornum = countNonZero(orange_roi);

    int threshold_network = 1000,network_on=0;
    if(orange_colornum>threshold_network){
        network_on = 1;
    }

    printf("%d %d", remote_on,network_on);
