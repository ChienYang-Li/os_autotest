...
static void decode_tlv(unsigned int spaces, unsigned int *tlv, unsigned int tlv_size)
{
	...
	case SND_CTL_TLVT_CHMAP_PAIRED:
		...
		while (size > 0) {
			printf("%s", snd_pcm_chmap_name((snd_pcm_chmap_position)(tlv[idx++])));
			size -= sizeof(unsigned int);
			if (size > 0)
				printf(",");
		}
	...
	if (level & LEVEL_BASIC) {
		...
		if (snd_mixer_selem_has_playback_volume(elem) ||
			...
			if (snd_mixer_selem_is_playback_mono(elem)) {
				...
				int first = 1;
				for (chn = (snd_mixer_selem_channel_id_t)0; chn <= SND_MIXER_SCHN_LAST; chn=(snd_mixer_selem_channel_id_t)(chn+1)){
...
static int sset_enum(snd_mixer_elem_t *elem, unsigned int argc, char **argv)
{
	...
	for (idx = 1; idx < argc; idx++) {
		...
		while (*ptr) {
			...
			if (ival < 0)
				return check_flag;
			if (snd_mixer_selem_set_enum_item(elem, (snd_mixer_selem_channel_id_t)(item++), ival) >= 0)
				check_flag = 1;
			/* skip separators */
